...
// start
//----------------------------------------------------------------------------
void Timer::start(int interval_ms, std::function<void(void)> callback_fn)
{
	// Assumes the timer is stopped if already running
...
// change_interval
//----------------------------------------------------------------------------
void Timer::change_interval(int interval_ms)
{
...
{
	std::chrono::duration<double> adj = std::chrono::milliseconds(0);
	...
	while (true)
	{
		...
		auto const timeout = std::chrono::steady_clock::now() + std::chrono::milliseconds(_interval_ms) - adj;
